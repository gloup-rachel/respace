...
분석은 CATEGORY 1·2·3 체크리스트 순서대로 수행하되, **출력은 아래 7섹션 구조로 재배치**하세요.
이슈가 없는 섹션은 해당 항목을 생략하고 "✅ 이상 없음"으로 표시하세요.

---
## 📬 리스페이스 뉴스레터 검수 결과
**호**: [주제명]
**타겟 페르소나**: [Core A / B / C / Strategic A / B] (운영자 지정 시 "(운영자 지정)" 병기, 미지정 시 AI 판단 근거 한 줄 추가)

---

### 1. 통합 피드백

*전체를 읽은 뒤 타겟 페르소나 관점에서 내리는 정성적 총평. 아래 세 가지를 반드시 포함하세요.*

**① 전반적 어조**
[B2B 전문가 톤인지, 광고 카피 느낌이 섞였는지, 분석적인지 선언적인지 — 2문장 이내]

**② 반복·과잉 표현**
[뉴스레터 전체에서 동일 단어·문구·구조가 반복되는 패턴을 정리. 없으면 "반복 패턴 없음"]

**③ 페르소나 관점 독해**
[타겟 페르소나가 이 뉴스레터를 읽었을 때 — 실무에 쓸 수 있는 정보가 있는지, 리스페이스가 전문 파트너로 인식되는지, 다음 행동(CTA)으로 자연스럽게 이어지는지 — 2~3문장]

---

### 2. 메일 제목

[이슈 있으면: 🔴/🟡 + 위치 인용 + 수정 방향 한 줄]
[이슈 없으면: ✅ 이상 없음]
...
[발신 이메일이 business@respace.co.kr과 일치하는지 확인]
[이슈 있으면: 🔴 + 실제 표기 인용 + "business@respace.co.kr로 통일"]
[이슈 없으면: ✅ 이상 없음]
...
*섹션별로 이슈가 있는 것만 항목으로 나열. 섹션 제목은 뉴스레터 원문 그대로 사용.*
...
*(이하 섹션별 반복. 이슈 없는 섹션은 생략)*

---

### 6. 마무리 / Final Note

[마지막 섹션이 본문 재요약인지 판단 (CATEGORY 3-D 결과 반영)]
[이슈 있으면: 🟡 + 재요약 여부 판단 + 삭제 또는 대체 방향 구체 제안]
[이슈 없으면: ✅ 이상 없음]
...
### 💬 총평
[한 줄: 배포 가능 여부 + 지금 당장 반영해야 할 핵심 액션 1개. 🔴 없으면 "배포 가능 — [Quick Fix 항목 중 가장 중요한 것 1개]만 먼저 반영 권장"]
