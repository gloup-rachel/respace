...
## 최종 템플릿 변환 (transform-template, 2026-05-27 갱신)

인사이트 생성기 결과를 대시보드 반영 시점에 **4섹션 최종 템플릿**으로 가공 (이슈+액션 통합).
...
변환된 entry (3섹션):
  1. 통합 총평 Summary
     ├ (1) 매체별 성과 비교 표 (weekly-table)
     ├ (2) CPD 중심 효율 요약 카드
     └ (3) 5줄 이내 인사이트 요약
  2. 매체별 성과 및 액션
     ├ 이상 매체만 inline 카드(channel-block)로 동적 노출
     ├ 네이버 카드 (이상 시) · 구글 카드 (이상 시) · Meta 카드 (이상 시)
     └ 이상 매체 0개면 "전 매체 정상" 메시지 1줄
  3. 이 외 논의 필요 사항 (외부 요인 + 다음 주 체크 + 미팅 안건)
   ↓
...
### 이상 매체 0~3개 시나리오 (분량)
| 이상 매체 수 | 섹션 2 분량 | 전체 합계 | max 7,000 여유 |
|---|---|---|---|
| 0개 (전 매체 정상) | ~100자 | ~1,800자 | 3.9배 |
| 1개 | ~800자 | ~2,500자 | 2.8배 |
| 2개 | ~1,400자 | ~3,100자 | 2.3배 |
| 3개 | ~2,000자 | ~3,700자 | 1.6배 |
